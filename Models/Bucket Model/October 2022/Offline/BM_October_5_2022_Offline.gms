...
October 5 2022. Model version for offline execution.

  * Base model for acreage variation of riparian habitat with willow tree plantations.
...
Table   Yield_p(aguse,j,k)   Crop Yield (tons per acre)
                  ...
                  Pecans.flood    Veges.flood  forage.flood  willow_tree.flood
*-------------------------- use node rows (+) ----------------------------------------
...
Table   Price_p(aguse,j)    Crop Prices ($ per ton)
                   ...
                   Pecans          Veges        forage       willow_tree
...
lan_p(t,'willow_tree',k,aguse) = 0.001;  //essentially zero observed land in environmental crop
...
table us_mx_1906_p(t,w)  actual deliveries at Acequia Madre gauge -- data source Rector Dr. Alfredo Granados - NOTE: not used for computations
...
env_mult(t) = env_mul(t, '1'); // make all years to 1
...
* forcing environmental thousands of acres minimum
tacres_v.lo('ebid_u_f', 'willow_tree', k, t, p, w) = 5 * env_mult(t);    //thousands of acre * year-based schedule (yes or no for a given year)
tacres_v.lo('epid_u_f', 'willow_tree', k, t, p, w) = eps;
tacres_v.lo('mxid_u_f', 'willow_tree', k, t, p, w) = eps;
...
cost_af_flow_import_p, cost_af_flow_export_p, elas_p, urb_av_back_cost0_p, tx_proj_op_p, source_p, us_mx_1906_p, precip_rate_p, evap_rat_p, z_p,
